...
import {
  useCreatePollShareToken,
  usePollShareTokens,
  useRevokePollShareToken,
} from '~/services/admin.service'
import { Button } from '~/ui/button'
import { Separator } from '~/ui/separator'

type Props = {
  poll: { id: number; slug: string; title: string }
}

/**
 * Admin-side invite-link management for a poll. Same three actions
 * as the event `SharePanel`: create a link (plaintext revealed once),
 * copy it, revoke it. Unlike the event share link, a poll invite
 * link grants full view + vote access — see `assertPollAccess` in
 * `worker/index.ts`.
 */
export default function PollSharePanel({ poll }: Props) {
  const { data: tokens } = usePollShareTokens(poll.id)
  const { mutate: createToken, isPending: isCreating } =
    useCreatePollShareToken(poll.id)
  const { mutate: revokeToken, isPending: isRevoking } =
    useRevokePollShareToken(poll.id)
  ...
  function fullShareUrl(plaintext: string): string {
    return `${origin()}/abstimmung/${poll.slug}/${plaintext}`
  }
            ...
            <ul className="space-y-2">
              {tokens.map((t) => (
                <li
                  ...
                  data-testid={`poll-share-token-${t.id}`}
                  key={t.id}
                >
                  ...
                  {t.is_active && (
                    <Button
                      aria-label="Einladungslink aufheben"
                      className="text-xs text-beet-700 hover:bg-beet-700/10 hover:text-beet-700"
                      disabled={isRevoking}
                      onClick={() =>
                        revokeToken(t.id, {
                          onSuccess: () =>
                            toast.success('Einladungslink aufgehoben.'),
                          onError: () =>
    ...
    <div className="space-y-2 rounded-[1.25rem] bg-forest-900/4 p-4 ring-1 ring-inset ring-forest-900/8">
      <label className="block space-y-1.5">
        <span className="text-xs font-medium text-forest-700">
          Label (optional, z. B. „Nachbarin Frau Müller")
        </span>
        <input
          className={FIELD}
          maxLength={200}
          onChange={(e) => onLabelChange(e.target.value)}
          placeholder="Für wen ist dieser Link?"
          value={label}
        />
      </label>
      <div className="flex justify-end">
        <Button
          ...
          type="button"
        >
          {isCreating ? 'Wird erstellt …' : 'Einladungslink erzeugen'}
        </Button>
      </div>
    </div>
  )
    ...
    >
      ...
      <div className="space-y-2">
        <div className="flex items-stretch gap-2">
          <input
            aria-label="Vollständiger Einladungslink"
            className={cn(FIELD, 'flex-1 font-mono text-xs')}
            data-testid="poll-share-token-plaintext"
